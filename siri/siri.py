...
async def start(client, message):
    await message.reply_photo(photo=Config.START_IMG, caption=Config.START_MSG.format(message.from_user.mention),
         reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(BUTTON1, url=TELEGRAM)
                 ],[
...
def a(client, message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    m = message.reply('`sᴇᴀʀᴄʜɪɴɢ..ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ...`')
    ...
    try:
        ...
        rep = f'🎶 <b>ᴛɪᴛʟᴇ:</b> <a href="{link}">{title}</a>\n⌚ <b>ᴅᴜʀᴀᴛɪᴏɴ:</b> <code>{duration}</code>\n📻 <b>ᴜᴘʟᴏᴀᴅᴇᴅ ʙʏ:</b> <a href="https://t.me/Cinemabranthen">ᴄɪɴᴇᴍᴀ.ʙʀᴀɴᴛʜᴇɴ</a>'
        ...
        m.edit('**An internal Error Occured, Report This @GxNeo!!**')
        print(e)
    try:
